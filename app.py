...
st.caption("集成『数据质量检查与相关性诊断』、『全域地质数据增强』与『多算法高精度回判』。")
...
with st.sidebar:
    ...
    use_augmentation = st.checkbox("启用全局数据增强", value=True, help="先对真实样本进行地质扰动及井间物理插值扩充，再送入模型训练")
...
# ----------------- TAB 2: 增强建模与符合率 -----------------
with tab2:
    st.subheader("2.1 增强与拟合评价指标")
    st.write(f"当前样本分布：真实井数 **{n_real}** 口，全局增强后训练样本数 **{len(X_train)}** 个（扩增了 {len(X_train) // n_real} 倍）")
    ...
    st.markdown("---")
    st.subheader("2.2 实测 EUR vs 预测 EUR 交会图 (无井名文字遮挡)")
